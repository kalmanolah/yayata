...
class ColorSequencer {
    ...
    next() {
        let color = sequenceColours[this.idx]
        this.idx += 1
        return color
    }
}
...
const stringToColour = function(str) {
    let res = null
    // Convert all characters to charcodes
    res = str.split('').map(x => x.charCodeAt(0))
    // Add charcodes
    res = res.reduce((x, y) => x + y)
    // Determine index in sequenceColours list
    res = res % sequenceColours.length
    // Return colour
